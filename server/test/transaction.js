...
describe('Transaction', () => {
    ...
    it('Should not create a transaction if cashier does not exist', (done) => {
        const transaction = {
            cashier: 5,
            amount: 4000
        };
        chai.request(app)
        .post('/api/v1/transactions/23456546/credit')
        .send(transaction)
        .end((err, res) => {
          res.should.have.status(400);
          res.body.should.be.a('object');
    ...
    it('Should not create a credit transaction if amount is greater than balance', (done) => {
        const transaction = {
            cashier: 2,
            amount: 500000
        };
